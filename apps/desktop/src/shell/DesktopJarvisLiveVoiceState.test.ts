...
describe("DesktopJarvisLiveVoiceState bridge", () => {
  it("keeps the last valid report and ignores malformed ones", () => {
    ...
    const ipcMain = {
      on: vi.fn((channel: string, listener: (event: unknown, raw: unknown) => void) => {
        if (channel === JARVIS_LIVE_VOICE_STATE_CHANNEL) reportListener = listener;
      }),
      removeListener: vi.fn(),
    };
    const bridge = createDesktopJarvisLiveVoiceStateBridge(ipcMain);
