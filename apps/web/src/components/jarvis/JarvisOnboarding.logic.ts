...
/**
 * User-facing reason when the Device step save fails. The dialog keeps the
 * typed value so pressing Continue again retries the same rename.
 */
...
/**
 * Device step helper copy. Renaming from a remote view renames the connected
 * node for every client, so say so instead of implying a local-only label.
 */
...
export function canAutoOpenJarvisOnboarding(input: {
  ...
  return input.environmentReady && !input.attemptMade && !input.completionStored;
}
