...
import {
  ...
  JARVIS_LIVE_VOICE_DEFAULT_IDLE_TIMEOUT_MS,
  JARVIS_LIVE_VOICE_DEFAULT_MAX_SESSION_MS,
  type JarvisLiveVoiceAudioElement,
  type JarvisLiveVoiceBrowser,
  type JarvisLiveVoiceCloseReason,
  type JarvisLiveVoiceDataChannel,
  type JarvisLiveVoiceMediaStream,
  type JarvisLiveVoiceMediaTrack,
  type JarvisLiveVoicePeerConnection,
  type JarvisLiveVoiceStartResult,
  type JarvisLiveVoiceStartupStage,
  type JarvisLiveVoiceStatus,
} from "./JarvisLiveVoice.logic";

class FakeTrack implements JarvisLiveVoiceMediaTrack {
  stopped = false;
  stop() {
    this.stopped = true;
  }
...
class FakeStream implements JarvisLiveVoiceMediaStream {
...
class FakeChannel implements JarvisLiveVoiceDataChannel {
...
class FakeAudio implements JarvisLiveVoiceAudioElement {
  autoplay = false;
...
class FakePeer implements JarvisLiveVoicePeerConnection {
  ontrack: JarvisLiveVoicePeerConnection["ontrack"] = null;
...
function fixture(
  options: {
    ...
    readonly getUserMedia?: () => Promise<JarvisLiveVoiceMediaStream>;
  ...
  const statuses: JarvisLiveVoiceStatus[] = [];
  const failures: string[] = [];
  const closed: JarvisLiveVoiceCloseReason[] = [];
  const stages: Array<{ stage: JarvisLiveVoiceStartupStage; atMs: number }> = [];
  const browserCalls = { mic: 0, silent: 0 };
  const browser: JarvisLiveVoiceBrowser = {
...
describe("Jarvis live voice controller", () => {
  ...
  it("auto-closes after 60s without user speech and stays honest", async () => {
    ...
    try {
      ...
      expect(JARVIS_LIVE_VOICE_DEFAULT_IDLE_TIMEOUT_MS).toBe(60_000);
      await f.controller.start();
      f.peer.channel.emit(started);
  ...
  it("enforces a 10 minute hard cap", async () => {
    ...
    try {
      const f = fixture({ closeTimeoutMs: 1 });
      expect(JARVIS_LIVE_VOICE_DEFAULT_MAX_SESSION_MS).toBe(600_000);
      await f.controller.start();
      f.peer.channel.emit(started);
  ...
  it("cancels startup when the user stops during microphone access", async () => {
    const gate = deferred<JarvisLiveVoiceMediaStream>();
    const tracks = [new FakeTrack()];
    const f = fixture({
      closeTimeoutMs: 1,
  ...
  it("suppresses a late RPC answer after the user stopped", async () => {
    const gate = deferred<JarvisLiveVoiceStartResult>();
    const f = fixture({
      closeTimeoutMs: 1,
      start: () => gate.promise,
    });
  ...
  it("keeps the context builder bounded and reference-only", () => {
    ...
    const context = buildJarvisLiveVoiceContext({
      nodeLabels: ["desk"],
      projects: many,
      providerNames: ["Codex"],
    ...
    expect(
      buildJarvisLiveVoiceContext({ nodeLabels: [], projects: [], providerNames: [] }),
    ).toBeUndefined();
  });
