...
function Card({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "relative flex flex-col rounded-[var(--radius)] border bg-card text-card-foreground",
      className,
    ),
    "data-slot": "card",
  };
...
function CardFrame({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "[--clip-top:-1rem] [--clip-bottom:-1rem] *:data-[slot=card]:first:[--clip-top:1px] *:data-[slot=card]:last:[--clip-bottom:1px] flex flex-col relative rounded-[var(--radius)] border bg-card text-card-foreground *:data-[slot=card]:-m-px *:not-last:data-[slot=card]:rounded-b-xs *:not-first:data-[slot=card]:rounded-t-xs *:data-[slot=card]:[clip-path:inset(var(--clip-top)_1px_var(--clip-bottom)_1px_round_calc(var(--radius)-1px))] *:data-[slot=card]:shadow-none *:data-[slot=card]:before:hidden",
      className,
    ),
