...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx @absterrg0/circe@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx @absterrg0/circe@0.0.32-nightly.1 service update");
  expect(output).not.toContain("@absterrg0/circe@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx @absterrg0/circe@0.0.29 service update");
  expect(output).not.toContain("npx @absterrg0/circe@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "npx @absterrg0/circe@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx @absterrg0/circe@latest service update");
});
