...
const leaseOf = (outcome: { readonly status: string; readonly lease?: CircePreAcceptLease }) => {
...
describe("Circe pre-accept request cancellation", () => {
  it.effect("reports unknown for a key that was never tracked", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* cancelPreAccept(state, "circe:request:node:origin:interaction:missing"),
  ...
  it.effect("aborts the in-flight interpretation and never lets it commit", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:cancel-me";
  ...
  it.effect("shares one inference with a concurrent duplicate, no second run", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:double";
      const gate = yield* Deferred.make<void>();
      let runs = 0;
  ...
  it.effect("refused duplicate close never removes the original commit", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:repro-race";
  ...
  it.effect("owner interruption leaves no record and lets a retry run", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:owner-interrupt";
  ...
  it.effect("duplicate close during owner interruption stays unknown for late cancel", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:dup-interrupt";
  ...
  it.effect("pre-start cancel tombstones without running the later track", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:pre-start";
  ...
  it.effect("never runs a retry after a cancelled request", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:retry-cancelled";
      const gate = yield* Deferred.make<void>();
  ...
  it.effect("cancels an interpretation that finished but never reached dispatch", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:pre-dispatch";
  ...
  it.effect("makes a commit cancellation await the exact typed receipt", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:committing";
      const outcome = yield* trackPreAccept(state, key, Effect.succeed("interpreted"));
      expect(outcome.status).toBe("tracked");
      const ownerLease = leaseOf(outcome);
      expect(yield* beginCommit(state, ownerLease)).toEqual({ proceed: true });
  ...
  it.effect("answers unknown when the commit fails instead of claiming acceptance", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:failed-commit";
      const outcome = yield* trackPreAccept(state, key, Effect.succeed("interpreted"));
      expect(outcome.status).toBe("tracked");
      const ownerLease = leaseOf(outcome);
      expect(yield* beginCommit(state, ownerLease)).toEqual({ proceed: true });
  ...
  it.effect("late cancel after failure stays unknown and retry runs fresh", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:late-unknown";
      const outcome = yield* trackPreAccept(state, key, Effect.succeed("interpreted"));
      const ownerLease = leaseOf(outcome);
      expect(yield* beginCommit(state, ownerLease)).toEqual({ proceed: true });
      yield* closeCommit(state, ownerLease);
  ...
  it.effect("forgets requests that settle without dispatching work", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:clarify";
  ...
  it.effect("lets exactly one owner commit while shared duplicates stay cancelled", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:race";
      const first = yield* trackPreAccept(state, key, Effect.succeed("first"));
      const second = yield* trackPreAccept(state, key, Effect.succeed("second"));
      expect(first.status).toBe("tracked");
  ...
  it.effect("shares interpretation failure instead of running a second inference", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:shared-failure";
      const entered = yield* Deferred.make<void>();
      const release = yield* Deferred.make<void>();
      let runs = 0;
  ...
  it.effect("runs an identical retry after acceptance untracked for downstream reconcile", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState();
      const key = "circe:request:node:origin:interaction:retry-accepted";
      const first = yield* trackPreAccept(state, key, Effect.succeed("first"));
      expect(first.status).toBe("tracked");
      const ownerLease = leaseOf(first);
      expect(yield* beginCommit(state, ownerLease)).toEqual({ proceed: true });
  ...
  it.effect("bounds the settled outcome memory", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState(2);
      for (const suffix of ["one", "two", "three"]) {
        const key = `circe:request:node:origin:interaction:${suffix}`;
        const gate = yield* Deferred.make<void>();
        const fiber = yield* Effect.forkChild(trackPreAccept(state, key, Deferred.await(gate)));
        yield* Effect.yieldNow;
        expect(yield* cancelPreAccept(state, key)).toEqual({ status: "cancelled" });
      ...
      expect(yield* cancelPreAccept(state, "circe:request:node:origin:interaction:one")).toEqual({
        status: "unknown",
      });
      expect(yield* cancelPreAccept(state, "circe:request:node:origin:interaction:three")).toEqual({
        status: "cancelled",
      });
    }),
  );
  ...
  it.effect("bounds the pre-register tombstone memory", () =>
    Effect.gen(function* () {
      const state = yield* makeCirceRequestCancellationState(2);
      const one = "circe:request:node:origin:interaction:pre-one";
      const two = "circe:request:node:origin:interaction:pre-two";
      const three = "circe:request:node:origin:interaction:pre-three";
