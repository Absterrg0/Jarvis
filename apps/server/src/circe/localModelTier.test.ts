...
import { CirceLocalModel, isLocalModelQualityEligible } from "./Services/CirceLocalModel.ts";
import {
  CirceLocalModelLive,
  defaultInferenceScriptForDirname,
  resolveLocalModelOutcome,
  type RawExtractorJson,
} from "./Layers/CirceLocalModel.ts";
...
const providerProposal: CirceSemanticProposal = {
  action: "start",
  refs: [],
  model: null,
  effort: null,
...
const configLayerFor = (input: FixtureInput) =>
  Layer.effect(
    ...
    Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "circe-local-tier-test-" })),
  );
...
const localFor = (
  handler: (command: ChildProcess.StandardCommand) => ReturnType<typeof mockHandle>,
) => CirceLocalModelLive.pipe(Layer.provide(spawnerLayerFor(handler)));
...
const localSatisfiedFor = (
  ...
  // CirceLocalModel while the config underneath carries file-system errors.
  localFor(handler).pipe(Layer.provide(NodeServices.layer), Layer.provide(withConfig(input)));

const providerLayerFor = (
  counter: { calls: number },
  proposal: CirceSemanticProposal = providerProposal,
) => {
...
const interpreterWith = <E2>(
  counter: { calls: number },
  local: Layer.Layer<CirceLocalModel, E2, never>,
  proposal: CirceSemanticProposal = providerProposal,
) =>
  makeCirceControllerInterpreterLive(providerLayerFor(counter, proposal), local).pipe(
...
const writeFixture = (
  ...
  Effect.gen(function* () {
    const dir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "circe-local-tier-" });
  ...
  it.effect("walks five levels from the Layers directory", () =>
    Effect.sync(() => {
      const resolved = defaultInferenceScriptForDirname(
        "/repo/apps/server/src/circe/Layers",
        (...parts) => NodePath.posix.resolve(...parts),
      );
      assert.equal(resolved, "/repo/scripts/circe-extract/inference.py");
...
describe("local model spawns the real layer command", () => {
  it.effect("passes --int8 with the required --policy and the verbatim source", () =>
    Effect.gen(function* () {
      ...
      const outcome = yield* Effect.flatMap(CirceLocalModel, (model) =>
      ...
      assert.equal(script.endsWith("scripts/circe-extract/inference.py"), true);
  ...
  it.effect("stays disabled through the actual layer without touching a child", () =>
    Effect.gen(function* () {
      let spawns = 0;
      const local = CirceLocalModelLive.pipe(
        Layer.provide(
          spawnerLayerFor(() => {
            spawns += 1;
        ...
        Layer.provide(NodeServices.layer),
        Layer.provideMerge(
          ServerConfig.layerTest(process.cwd(), { prefix: "circe-local-tier-disabled-" }),
        ),
      );
  ...
  it.effect("interrupts the owned child scope on cancellation", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* Effect.forkChild(
        Effect.flatMap(CirceLocalModel, (model) => model.infer({ source: MULTIWORD })).pipe(
...
describe("local model cascade through the controller route", () => {
  ...
  it.effect("cancels a pre-registered request without running local inference", () =>
    Effect.gen(function* () {
      ...
      const counter = { calls: 0 };
      const key = "circe:request:node:origin:interaction:local-model-cancel-probe";
...
describe("offline inference protocol probe (one bounded real call, never production)", () => {
  it.live("the stable candidate answers the deployment CLI shape", () =>
    Effect.gen(function* () {
      ...
      // Set CIRCE_LOCAL_PROBE_LIVE=1 for the single offline verification.
      if ((process.env.CIRCE_LOCAL_PROBE_LIVE ?? "").trim() !== "1") {
        return;
      }
      const explicitDir = (process.env.CIRCE_LOCAL_PROBE_MODEL_DIR ?? "").trim();
      const explicitPython = (process.env.CIRCE_LOCAL_PROBE_PYTHON ?? "").trim();
      const candidateDir =
        explicitDir.length > 0
          ? explicitDir
          : "/home/abstergo/.local/share/circe-experiments/circe-v08/v08-small-s7";
      const python =
