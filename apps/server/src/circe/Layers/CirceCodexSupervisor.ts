...
import {
  CIRCE_CODEX_OAUTH_CLIENT_ID,
  CIRCE_CODEX_OAUTH_SCOPE,
  CIRCE_CODEX_ORIGINATOR,
  CIRCE_CODEX_RESPONSES_ENDPOINT,
  CIRCE_CODEX_SUPERVISOR_DEFAULT_MODEL,
  CIRCE_CODEX_SUPERVISOR_MAX_PROMPT_CHARS,
  CIRCE_CODEX_TOKEN_ENDPOINT,
  ...
  parseCirceCodexAuth,
  type CirceCodexCredentials,
  type CirceCodexSupervisorAvailability,
  type CirceCodexSupervisorOutcome,
  type ParsedCirceCodexAuth,
...
interface ResolvedCodexAuth {
  ...
  readonly credentials: CirceCodexCredentials | null;
}

export interface CirceCodexSupervisorLiveOptions {
  /** Explicit auth file candidates; `CIRCE_CODEX_AUTH_FILE` and defaults still rank first. */
 ...
 * auth read.
 */
export const makeCirceCodexSupervisorLive = (
  options: CirceCodexSupervisorLiveOptions = {},
): Layer.Layer<CirceCodexSupervisor, never, FileSystem.FileSystem | Path.Path> =>
  Layer.effect(
    CirceCodexSupervisor,
    Effect.gen(function* () {
      ...
      const endpoint = options.endpoint ?? CIRCE_CODEX_RESPONSES_ENDPOINT;
      const tokenEndpoint = options.tokenEndpoint ?? CIRCE_CODEX_TOKEN_ENDPOINT;
      const originator = options.originator ?? CIRCE_CODEX_ORIGINATOR;
      ...
      const model =
        options.model?.trim() ||
        process.env.CIRCE_CODEX_SUPERVISOR_MODEL?.trim() ||
        CIRCE_CODEX_SUPERVISOR_DEFAULT_MODEL;
      ...
      const authFileCandidates = (): ReadonlyArray<string> => {
        ...
        push(process.env.CIRCE_CODEX_AUTH_FILE);
      ...
      const refreshCredentials = (
        filePath: string,
        parsed: ParsedCirceCodexAuth,
        original: unknown,
      ): Effect.Effect<CirceCodexCredentials | null> =>
        Effect.gen(function* () {
          const refreshToken = parsed.credentials.refreshToken;
          if (refreshToken === undefined) return null;
          const body = new URLSearchParams({
            client_id: CIRCE_CODEX_OAUTH_CLIENT_ID,
            grant_type: "refresh_token",
            refresh_token: refreshToken,
            scope: CIRCE_CODEX_OAUTH_SCOPE,
          });
          ...
          const merged = mergeCirceCodexRefreshedAuth({
            shape: parsed.shape,
            original,
            accessToken,
          ...
          return {
            ...
            ...(parsed.credentials.accountId === undefined
              ...
              : { accountId: parsed.credentials.accountId }),
            ...(expiresAtMs === undefined ? {} : { expiresAtMs }),
          } satisfies CirceCodexCredentials;
        });
      ...
      const resolveUncached = Effect.gen(function* () {
        ...
        for (const filePath of authFileCandidates()) {
          ...
          const parsed = parseCirceCodexAuth(json);
          if (parsed === null) continue;
          let credentials = parsed.credentials;
          if (!isCirceCodexCredentialFresh(credentials, nowMs, refreshSkewMs)) {
        ...
        Effect.gen(function* () {
          ...
          if (response === null) {
            yield* Effect.logWarning(
              "Circe codex supervisor request failed or timed out; falling back.",
            );
          ...
          if (!response.ok) {
            ...
            yield* Effect.logDebug(
              `Circe codex supervisor HTTP ${response.status}: ${(detail ?? "").slice(0, 200)}`,
            );
            return { status: "decline", reason: "codex-supervisor-error" } as const;
          }
          const streamed = yield* readCirceCodexStream(response);
          if (streamed === null) {
      ...
      return {
        availability: resolve.pipe(
          Effect.map((resolved): CirceCodexSupervisorAvailability =>
 ...
 */
const readCirceCodexStream = (response: Response): Effect.Effect<string | null> =>
  Effect.tryPromise({
    try: async () => {
...
export const CirceCodexSupervisorLive = makeCirceCodexSupervisorLive();
