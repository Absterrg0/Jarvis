...
import {
  CIRCE_OPENCODE_SUPERVISOR_MAX_PROMPT_CHARS,
  CIRCE_OPENCODE_SESSION_HEADER,
  ...
  type CirceOpencodeAuth,
  type CirceOpencodeSupervisorAvailability,
  type CirceOpencodeSupervisorOutcome,
} from "../Services/CirceOpencodeSupervisor.ts";
import { extractCirceCodexJsonObject } from "../Services/CirceCodexSupervisor.ts";
...
export interface CirceOpencodeSupervisorLiveOptions {
 ...
 */
export const makeCirceOpencodeSupervisorLive = (
  options: CirceOpencodeSupervisorLiveOptions = {},
): Layer.Layer<CirceOpencodeSupervisor, never, FileSystem.FileSystem | Path.Path> =>
  Layer.effect(
    CirceOpencodeSupervisor,
    Effect.gen(function* () {
      ...
      const authFileCandidates = (): ReadonlyArray<string> => {
        ...
        push(process.env.CIRCE_OPENCODE_AUTH_FILE);
      ...
      const resolve = Effect.gen(function* () {
        ...
        for (const filePath of authFileCandidates()) {
          ...
          const auth = parseCirceOpencodeAuth(json);
      ...
      const interpret = (input: {
        ...
        Effect.gen(function* () {
          ...
          if (
            ...
            prompt.length > CIRCE_OPENCODE_SUPERVISOR_MAX_PROMPT_CHARS
          ) {
            return { status: "decline", reason: "opencode-supervisor-error" } as const;
          }
          const resolved = yield* resolve;
          const route = resolveCirceOpencodeRoute({
            auth: resolved.auth,
            ...
            try: () =>
              doFetch(`${route.baseUrl}/chat/completions`, {
                ...
                headers: {
                  ...
                  [CIRCE_OPENCODE_SESSION_HEADER]: NodeCrypto.randomUUID(),
                },
          ...
          if (response === null) {
            yield* Effect.logWarning(
              "Circe opencode supervisor request failed or timed out; falling back.",
            );
          ...
          if (!response.ok) {
            ...
            yield* Effect.logDebug(
              `Circe opencode supervisor HTTP ${response.status}: ${(detail ?? "").slice(0, 200)}`,
            );
            return { status: "decline", reason: "opencode-supervisor-error" } as const;
          }
          const streamed = yield* readCirceOpencodeStream(response);
          if (streamed === null) {
      ...
      return {
        availability: resolve.pipe(
          Effect.map((resolved): CirceOpencodeSupervisorAvailability => {
            const route = resolveCirceOpencodeRoute({
              auth: resolved.auth,
              ...(options.goModel === undefined ? {} : { goModel: options.goModel }),
              ...(options.zenModel === undefined ? {} : { zenModel: options.zenModel }),
...
const readCirceOpencodeStream = (response: Response): Effect.Effect<string | null> =>
  Effect.tryPromise({
    try: async () => {
      ...
      for await (const chunk of body as unknown as AsyncIterable<Uint8Array>) {
        ...
        while (newline >= 0) {
          ...
          const parsed = interpretCirceOpencodeSseEvent(event);
          if (parsed.type === "text") {
            text += parsed.delta;
            if (extractCirceCodexJsonObject(text) !== null) return text;
          }
        }
        if (done) break;
      }
...
export const CirceOpencodeSupervisorLive = makeCirceOpencodeSupervisorLive();
