...
import {
  CIRCE_FAST_SUPERVISOR_MAX_PROMPT_CHARS,
  ...
  parseFxStatusOutput,
  type CirceFastSupervisorAvailability,
  type CirceFastSupervisorOutcome,
...
const SUPERVISOR_WORKSPACE_DIR = "circe-fx-supervisor";
...
interface ResolvedFx {
  ...
  readonly availability: CirceFastSupervisorAvailability;
}
...
export interface CirceFastSupervisorLiveOptions {
  /** Explicit binary path; `CIRCE_FX_BINARY` and `~/.fx/bin/fx` still rank first. */
 ...
 */
export const makeCirceFastSupervisorLive = (
  options: CirceFastSupervisorLiveOptions = {},
): Layer.Layer<CirceFastSupervisor, never, FileSystem.FileSystem | Path.Path> =>
  Layer.effect(
    CirceFastSupervisor,
    Effect.gen(function* () {
      ...
      const candidates = (): ReadonlyArray<string> => {
        ...
        push(process.env.CIRCE_FX_BINARY);
      ...
      const resolve = Effect.gen(function* () {
        ...
        for (const candidate of candidates()) {
          ...
          const availability: CirceFastSupervisorAvailability = isFxStatusAuthenticated(summary)
      ...
      const interpret = (input: {
        ...
        Effect.gen(function* () {
          ...
          if (
            ...
            prompt.length > CIRCE_FAST_SUPERVISOR_MAX_PROMPT_CHARS
          ) {
          ...
          if (collected === null) {
            yield* Effect.logDebug("Circe fast supervisor timed out; falling back to providers.");
            return { status: "decline", reason: "fast-supervisor-timeout" } as const;
          }
          if (collected.capped || collected.exitCode !== 0) {
            yield* Effect.logDebug("Circe fast supervisor failed; falling back to providers.", {
          ...
          if (assistantText === null) {
            yield* Effect.logDebug("Circe fast supervisor returned no assistant text.");
            return { status: "decline", reason: "fast-supervisor-malformed" } as const;
          }
          const raw = extractJsonObjectFromText(assistantText);
          if (raw === null) {
            yield* Effect.logDebug("Circe fast supervisor returned no JSON object.");
            return { status: "decline", reason: "fast-supervisor-malformed" } as const;
          }
          ...
          if (proposal === null) {
            yield* Effect.logDebug("Circe fast supervisor returned an invalid proposal schema.");
            return { status: "decline", reason: "fast-supervisor-malformed" } as const;
          }
...
export const CirceFastSupervisorLive = makeCirceFastSupervisorLive();
