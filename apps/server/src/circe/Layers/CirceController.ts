...
function buildMeshSemanticPrompt(input: {
  ...
  return [
    ...
    "Quick actions take no project or task. A weather or local-time question uses action lookup with lookup {kind: weather|time, location, day: now|today|tomorrow}; copy location verbatim from the transcript and use day now unless the user says today or tomorrow. A request to open a site uses action open-website with website set to the named site or URL. Never use lookup or open-website for work that edits, deploys, or investigates a project.",
