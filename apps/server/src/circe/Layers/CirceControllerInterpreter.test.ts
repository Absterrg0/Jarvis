...
import { CirceCodexSupervisor } from "../Services/CirceCodexSupervisor.ts";
import { makeCirceControllerInterpreterLive } from "./CirceController.ts";
import { CIRCE_SEMANTIC_UNAVAILABLE_PROMPT } from "../controllerHelpers.ts";
...
const codexSupervisorLayer = (
  ...
  Layer.succeed(CirceCodexSupervisor, {
...
describe("CirceControllerInterpreter fallback", () => {
  ...
  it.effect("returns an honest prompt when all candidates fail", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(result.prompt).toBe(CIRCE_SEMANTIC_UNAVAILABLE_PROMPT);
