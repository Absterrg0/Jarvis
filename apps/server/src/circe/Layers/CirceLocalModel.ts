...
import {
  CIRCE_LOCAL_MODEL_DEFAULT,
  CirceLocalModel,
  type CirceLocalModelConfig,
  type CirceLocalModelOutcome,
  isLocalModelQualityEligible,
} from "../Services/CirceLocalModel.ts";
...
const INFERENCE_SCRIPT_PARTS = ["scripts", "circe-extract", "inference.py"] as const;
/** Levels from `apps/server/src/circe/Layers/` up to the repo root. */
 ...
 * math, pinned by unit test: `apps/server/src/circe/Layers/` is five levels
 * below the repo root, so four would land on the nonexistent
 * `apps/scripts/circe-extract/inference.py`.
 */
...
const readConfig = (server: ServerConfig.ServerConfig["Service"]): CirceLocalModelConfig => {
  const configured = server.circeLocalModel;
  if (configured === undefined) return CIRCE_LOCAL_MODEL_DEFAULT;
  return {
    enabled: configured.enabled === true,
    modelDir: configured.modelDir ?? "",
    pythonBin: configured.pythonBin ?? CIRCE_LOCAL_MODEL_DEFAULT.pythonBin,
    timeoutMs: configured.timeoutMs ?? CIRCE_LOCAL_MODEL_DEFAULT.timeoutMs,
...
export const CirceLocalModelDisabledLive = Layer.succeed(CirceLocalModel, {
  infer: () => Effect.succeed({ status: "decline", reason: "local-model-disabled" } as const),
});

export const CirceLocalModelLive = Layer.effect(
  CirceLocalModel,
  Effect.gen(function* () {
    ...
    const infer = (request: { readonly source: string }): Effect.Effect<CirceLocalModelOutcome> => {
      ...
      return Effect.gen(function* () {
        ...
        if (reportRaw === null) {
          yield* Effect.logWarning("Circe local model report unreadable; declining to provider.", {
            reportPath,
          });
        ...
        if (
          ...
          yield* Effect.logWarning(
            "Circe local model quality gate failed; declining to provider.",
            {
              reportPath,
            },
          );
        ...
        if (!policyExists) {
          yield* Effect.logWarning("Circe local model policy missing; declining to provider.", {
            policyPath,
          });
          return { status: "decline", reason: "quality-gate-failed" };
        }
        ...
        if (collected === "failed") {
          yield* Effect.logWarning("Circe local model failed; declining to provider.");
          return { status: "decline", reason: "local-model-error" };
        }
        if (Option.isNone(collected)) {
          yield* Effect.logWarning("Circe local model timed out; declining to provider.", {
        ...
        if (capped) {
          yield* Effect.logWarning("Circe local model output too large; declining to provider.");
          return { status: "decline", reason: "local-model-error" };
        }
        if (exitCode !== 0) {
          yield* Effect.logWarning("Circe local model exited nonzero; declining to provider.", {
            exitCode,
          });
          return { status: "decline", reason: "local-model-error" };
        }
        ...
        if (parsed === null || typeof parsed !== "object") {
          yield* Effect.logWarning(
            "Circe local model returned invalid JSON; declining to provider.",
          );
          return { status: "decline", reason: "local-model-error" };
        }
        ...
        if (outcome.reason === "text-mismatch") {
          yield* Effect.logWarning("Circe local model text mismatch; declining to provider.");
        } else if (outcome.reason === "local-model-abstained") {
          yield* Effect.logWarning("Circe local model abstained; declining to provider.");
        } else {
          yield* Effect.logWarning(
            "Circe local model returned malformed spans; declining to provider.",
          );
        }
        return outcome;
      });
