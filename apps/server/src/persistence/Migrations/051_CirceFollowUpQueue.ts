...
export default Effect.gen(function* () {
  ...
  yield* sql`
    CREATE TABLE IF NOT EXISTS circe_follow_up_queue (
  ...
  yield* sql`
    CREATE INDEX IF NOT EXISTS idx_circe_follow_up_queue_pending
    ON circe_follow_up_queue(status, position)
  `;

  yield* sql`
    CREATE INDEX IF NOT EXISTS idx_circe_follow_up_queue_thread
  ...
  yield* sql`
    ...
    SELECT
      queued.activity_id,
      'circe:queue:dispatch:' || queued.activity_id,
      queued.thread_id,
      threads.project_id,
      queued.summary,
      'pending',
