...
it("normalizes Retina pixels and a negative-origin selected monitor to the pointer grid", () => {
  ...
  const frame = PNG.sync.read(
    Buffer.from(normalizeFrame(PNG.sync.write(png), display, [display, other], "desktop")),
  );
  expect([frame.width, frame.height]).toEqual([2, 2]);
  for (let i = 0; i < frame.data.length; i += 4)
    expect([...frame.data.subarray(i, i + 4)]).toEqual([255, 0, 0, 255]);
});
it("rejects mismatched geometry and truncated PNG data", () => {
  const display = { id: "main", x: 0, y: 0, width: 10, height: 10, scale: 1, primary: true };
  const png = PNG.sync.write(new PNG({ width: 20, height: 10 }));
  expect(() => normalizeFrame(png, display, [display], "display")).toThrow("geometry");
  expect(() => normalizeFrame(png.subarray(0, 33), display, [display], "display")).toThrow();
});
