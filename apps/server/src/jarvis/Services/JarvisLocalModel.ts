...
import type { ExtractionAdapterRejectionReason } from "@t3tools/jarvis-core/extraction";
 ...
 */
export interface JarvisLocalModelConfig {
...
export const JARVIS_LOCAL_MODEL_DEFAULT: JarvisLocalModelConfig = {
  enabled: false,
  modelDir: "",
  pythonBin: "python3",
  timeoutMs: 8_000,
...
export type JarvisLocalModelDeclineReason =
...
export type JarvisLocalModelOutcome =
  | { readonly status: "proposal"; readonly proposal: JarvisSemanticProposal }
  | { readonly status: "decline"; readonly reason: JarvisLocalModelDeclineReason }
  | {
 ...
 */
export class JarvisLocalModel extends Context.Service<
  JarvisLocalModel,
  {
    readonly infer: (input: { readonly source: string }) => Effect.Effect<JarvisLocalModelOutcome>;
  }
>()("t3/jarvis/Services/JarvisLocalModel") {}

/** Frozen deployment gate, mirrored from scripts/jarvis-extract/evaluate.py. */
export const JARVIS_LOCAL_MODEL_QUALITY_THRESHOLDS = {
...
const metricsPass = (metrics: QualityTestMetrics): boolean => {
  const t = JARVIS_LOCAL_MODEL_QUALITY_THRESHOLDS;
