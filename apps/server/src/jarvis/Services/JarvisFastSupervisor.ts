 ...
 */
export type JarvisFastSupervisorDeclineReason =
...
export type JarvisFastSupervisorOutcome =
  | { readonly status: "proposal"; readonly proposal: JarvisSemanticProposal }
  | { readonly status: "decline"; readonly reason: JarvisFastSupervisorDeclineReason };

export interface JarvisFastSupervisorAvailability {
  readonly available: boolean;
  /** Active fx model, for family matching against the configured supervisor. */
  readonly model?: string;
}

export class JarvisFastSupervisor extends Context.Service<
  JarvisFastSupervisor,
  {
    readonly availability: Effect.Effect<JarvisFastSupervisorAvailability>;
...
>()("t3/jarvis/Services/JarvisFastSupervisor") {}

/** Request bodies for one supervisor turn are bounded like the provider prompt. */
export const JARVIS_FAST_SUPERVISOR_MAX_PROMPT_CHARS = 32_000;
 ...
 */
export const JARVIS_FAST_SUPERVISOR_SYSTEM_PROMPT = [
  "You are a strict semantic router for ARIS. Reply with exactly one JSON object and nothing else: no prose, no markdown, no code fences, no tools.",
