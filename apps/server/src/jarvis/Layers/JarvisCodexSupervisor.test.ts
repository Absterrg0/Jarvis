...
import {
  ...
  jarvisCodexAccountIdFromIdToken,
  mergeJarvisCodexRefreshedAuth,
  parseJarvisCodexAuth,
} from "../Services/JarvisCodexSupervisor.ts";
import { makeJarvisCodexSupervisorLive } from "./JarvisCodexSupervisor.ts";
...
describe("Jarvis codex supervisor helpers", () => {
  it("parses both Codex CLI and fx auth shapes", () => {
    expect(
      parseJarvisCodexAuth({ tokens: { access_token: "tok", refresh_token: "ref" } }),
    ).toMatchObject({ shape: "codex", credentials: { accessToken: "tok", refreshToken: "ref" } });
    expect(
      parseJarvisCodexAuth({ access_token: "tok", refresh_token: "ref", expires_at_ms: 100 }),
    ).toMatchObject({
      shape: "fx",
      credentials: { accessToken: "tok", refreshToken: "ref", expiresAtMs: 100 },
    });
    expect(parseJarvisCodexAuth({ tokens: {} })).toBeNull();
    expect(parseJarvisCodexAuth(null)).toBeNull();
  });

  it("reads the account id and expiry from JWT claims", () => {
    const idToken = jwt({ "https://api.openai.com/auth": { chatgpt_account_id: "acct_1" } });
    expect(jarvisCodexAccountIdFromIdToken(idToken)).toBe("acct_1");
    expect(jarvisCodexAccountIdFromIdToken("not-a-jwt")).toBeNull();
    const access = jwt({ exp: 2_000_000 });
    expect(parseJarvisCodexAuth({ access_token: access })?.credentials.expiresAtMs).toBe(
      2_000_000_000,
    );
  });

  it("treats unknown expiry as fresh and enforces the skew window", () => {
    expect(isJarvisCodexCredentialFresh({ accessToken: "t" }, 10, 60)).toBe(true);
    expect(isJarvisCodexCredentialFresh({ accessToken: "t", expiresAtMs: 1_000 }, 950, 60)).toBe(
      false,
    );
    expect(isJarvisCodexCredentialFresh({ accessToken: "t", expiresAtMs: 2_000 }, 950, 60)).toBe(
      true,
    );
  });

  it("merges refreshed tokens back into each shape", () => {
    const codex = mergeJarvisCodexRefreshedAuth({
      shape: "codex",
    ...
    const fx = mergeJarvisCodexRefreshedAuth({
      shape: "fx",
      original: { version: 1 },
      accessToken: "new",
      refreshToken: "new-ref",
  ...
  it("translates SSE events and extracts the first JSON object", () => {
    expect(
      interpretJarvisCodexSseEvent({ type: "response.output_text.delta", delta: "hi" }),
    ...
    expect(
      interpretJarvisCodexSseEvent({ type: "response.failed", error: { message: "x" } }),
    ).toEqual({ type: "failed", message: "x" });
    expect(extractJarvisCodexJsonObject('noise {"a":{"b":1}} tail')).toEqual({ a: { b: 1 } });
    expect(extractJarvisCodexJsonObject("no json")).toBeNull();
  });
});
