...
const withLayer = <A, E>(
  run: (supervisor: JarvisGrokSupervisor["Service"]) => Effect.Effect<A, E>,
) =>
  Effect.gen(function* () {
    ...
    const layer = makeJarvisGrokSupervisorLive({
      authFiles: [authFile],
    ...
    const supervisor = yield* JarvisGrokSupervisor.pipe(Effect.provide(layer));
    return yield* run(supervisor);
  }).pipe(Effect.provide(NodeServices.layer));

describe("Jarvis grok supervisor helpers", () => {
  ...
  it("builds a tool-free body without tool_choice", () => {
    const body = buildJarvisGrokResponsesBody({
      model: "grok-4.6",
      prompt: "route",
      instructions: "schema",
    });
...
describe("Jarvis grok supervisor layer", () => {
