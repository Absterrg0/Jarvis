...
import { makeJarvisFastSupervisorLive } from "./JarvisFastSupervisor.ts";
...
const withStub = <A, E>(
  script: string,
  run: (supervisor: JarvisFastSupervisor["Service"]) => Effect.Effect<A, E>,
) =>
  Effect.gen(function* () {
...
describe("Jarvis fast supervisor", () => {
  ...
  it("states the proposal schema in the fx base prompt", () => {
    ...
    expect(JARVIS_FAST_SUPERVISOR_SYSTEM_PROMPT).toContain('"action"');
    expect(JARVIS_FAST_SUPERVISOR_SYSTEM_PROMPT).toContain('"refs"');
    expect(JARVIS_FAST_SUPERVISOR_SYSTEM_PROMPT).toContain(
