...
import { ensureJarvisConversationsProject } from "./conversationsProject.ts";
...
describe("ensureJarvisConversationsProject", () => {
  it.effect("fails without creating the project when the guidance file cannot be written", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* Effect.exit(
        ensureJarvisConversationsProject.pipe(
      ...
      Effect.provide(ServerConfig.layerTest(process.cwd(), { prefix: "jarvis-conv-fail-" })),
