...
import { FINAL_MAX_CALLS, loadFinalCorpus } from "./circeSemanticFinalLoader.ts";
...
function makeCase(id: string, family: string, utterance: string): Record<string, unknown> {
  return {
    ...
    expectedAck: "Request accepted for Circe.",
  };
}
  ...
  for (let index = 0; index < 40; index += 1) {
    ...
    cases.push(
      makeCase(
        ...
        family,
        `Fresh sealed utterance ${index} for Circe.`,
      ),
    );
  }
  return cases;
...
describe("sealed final loader", () => {
  it("reports missing when the external dir is absent", () => {
    const result = loadFinalCorpus(
      NodePath.join(NodeOS.tmpdir(), "circe-final-missing-xyz"),
      false,
    );
  ...
  it("rejects fewer than 40 cases and wrong reviewed status", () => {
    const dir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "circe-final-"));
    try {
