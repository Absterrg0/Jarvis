...
import { JARVIS_SEMANTIC_THRESHOLDS, scoreProposal } from "./jarvisSemanticEvalEngine.ts";
...
describe("Jarvis semantic eval corpus", () => {
  it("keeps the previous 92-case corpus as regression source", () => {
    expect(jarvisSemanticEvalCorpus).toHaveLength(92);
    expect(new Set(jarvisSemanticEvalCorpus.map((entry) => entry.id)).size).toBe(92);
  ...
  it("treats previous dev and heldout as one regression set", () => {
    const dev = jarvisSemanticEvalCorpus.filter((entry) => entry.split === "dev");
    const heldout = jarvisSemanticEvalCorpus.filter((entry) => entry.split === "heldout");
  ...
  it("skips legacy Intent snapshots under the new proposal schema, never passing them", () => {
    let legacy = 0;
    for (const entry of jarvisSemanticEvalCorpus) {
  ...
  it("pins stripped dispatch wording on the two destination dev cases", () => {
    const byId = new Map(jarvisSemanticEvalCorpus.map((entry) => [entry.id, entry]));
  ...
  it("builds a strict schema that tracks the live proposal", () => {
    const schema = buildStrictJarvisSemanticJsonSchema();
  ...
  it("keeps an independent 12-case dev regression with families and literal expectations", () => {
    ...
    const families = new Set(jarvisSemanticDevCorpus.map((entry) => entry.family));
  ...
  it("freezes user-approved thresholds", () => {
    expect(JARVIS_SEMANTIC_THRESHOLDS.exclusionDispatch).toBe(0);
    expect(JARVIS_SEMANTIC_THRESHOLDS.textCorruption).toBe(0);
    expect(JARVIS_SEMANTIC_THRESHOLDS.crossNodeViolation).toBe(0);
    expect(JARVIS_SEMANTIC_THRESHOLDS.staleSpeech).toBe(0);
    expect(JARVIS_SEMANTIC_THRESHOLDS.completePassRate).toBe(0.95);
    expect(JARVIS_SEMANTIC_THRESHOLDS.unnecessaryClarificationRate).toBe(0.05);
    expect(JARVIS_SEMANTIC_THRESHOLDS.ambiguityAccuracy).toBe(0.95);
    expect(JARVIS_SEMANTIC_THRESHOLDS.repeatConsistency).toBe(1);
  });
});
