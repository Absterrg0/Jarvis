...
export const FINAL_DIR_DEFAULT = "/tmp/opencode/circe-semantic-final";
...
export type FinalLoadResult =
  | {
      readonly status: "ok";
      readonly cases: ReadonlyArray<CirceSemanticEvalCaseV2>;
...
export function loadFinalCorpus(
  ...
  const cases: Array<CirceSemanticEvalCaseV2> = [];
  for (const [index, entry] of casesRaw.entries()) {
    ...
    const typed = entry as CirceSemanticEvalCaseV2;
  ...
  const expanded: Array<CirceSemanticEvalCaseV2> = [...cases];
