...
import { buildStrictJarvisSemanticJsonSchema } from "./jarvisSemanticSchema.ts";
import { jarvisSemanticEvalCorpus } from "./jarvisSemanticEvalCorpus.ts";
import { jarvisSemanticDevCorpus } from "./jarvisSemanticDevCorpus.ts";
import {
  buildEvalContext,
  gateThresholds,
  scoreProposal,
  type EvalResult,
  type JarvisSemanticEvalCaseV2,
} from "./jarvisSemanticEvalEngine.ts";
import {
  FINAL_DIR_DEFAULT,
  FINAL_MAX_CALLS,
  loadFinalCorpus,
} from "./jarvisSemanticFinalLoader.ts";

const USAGE = `usage: evalJarvisSemantic.ts [--split dev|regression|final|all] [--case <a,b,c>]
...
/tmp/opencode/jarvis-semantic-final); 40 fresh cases, plus 10x3 repeats with
...
function regressionCases(): Array<UnifiedCase> {
  return jarvisSemanticEvalCorpus.map((entry) => ({
    id: entry.id,
...
function devCases(): Array<UnifiedCase> {
  return jarvisSemanticDevCorpus.map((entry: JarvisSemanticEvalCaseV2) => ({
    id: entry.id,
    utterance: entry.utterance,
    action: entry.action,
    family: entry.family,
...
if (isLiveRun) {
  const pinned = process.env.JARVIS_SEMANTIC_EVAL_CODEX?.trim() ?? "";
  if (pinned.length === 0) {
    fail(
      'Live eval requires JARVIS_SEMANTIC_EVAL_CODEX to name the exact codex executable (e.g., JARVIS_SEMANTIC_EVAL_CODEX="$(volta which codex)"). Unset PATH lookup is disabled.',
    );
  }
  if (!NodePath.isAbsolute(pinned) || !NodeFS.existsSync(pinned)) {
    fail(
      `JARVIS_SEMANTIC_EVAL_CODEX must be an absolute existing executable path, got ${pinned}. Resolve with volta which codex.`,
    );
  }
}
...
NodeFS.writeFileSync(schemaPath, JSON.stringify(buildStrictJarvisSemanticJsonSchema()), "utf8");
...
async function runLiveProposal(
  ...
  const prompt = buildJarvisSemanticPrompt(context, prepared);
  ...
  const evalBinary = process.env.JARVIS_SEMANTIC_EVAL_CODEX?.trim() ?? "";
  if (evalBinary.length === 0) {
    throw new Error(
      "Live eval requires JARVIS_SEMANTIC_EVAL_CODEX to name the exact codex executable. Resolve with volta which codex.",
    );
  }
  ...
  const program = Effect.gen(function* () {
    ...
    const cwd = yield* fileSystem.makeTempDirectoryScoped({ prefix: "jarvis-semantic-" });
...
try {
  for (const [index, entry] of selectedCases.entries()) {
    ...
    if (keepArtifacts) {
      NodeFS.writeFileSync(
        ...
        buildJarvisSemanticPrompt(context, prepared),
        "utf8",
      );
    }
    try {
  ...
  if (keepArtifacts) {
    try {
      ...
      const keepDir = NodePath.join(artifactsDir, `jarvis-semantic-eval-${stamp()}`);
...
try {
  NodeFS.mkdirSync(artifactsDir, { recursive: true });
  NodeFS.writeFileSync(
    NodePath.join(artifactsDir, `jarvis-semantic-eval-report-${stamp()}.json`),
