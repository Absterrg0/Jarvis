 ...
 */
export function buildStrictJarvisSemanticJsonSchema(): JsonObject {
  const base = toJsonSchemaObject(JarvisSemanticProposal);
  if (!isObject(base) || !isObject(base.properties)) {
    throw new Error("JarvisSemanticProposal did not produce an object schema.");
  }
