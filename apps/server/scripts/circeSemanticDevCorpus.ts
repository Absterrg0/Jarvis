import type { CirceSemanticEvalCaseV2 } from "./circeSemanticEvalEngine.ts";
import type { SemanticRef, SemanticRole } from "@circe/core/command";
...
function proposal(
  action: CirceSemanticEvalCaseV2["action"],
  source: string,
 ...
 */
export const circeSemanticDevCorpus: ReadonlyArray<CirceSemanticEvalCaseV2> = [
  {
    id: "dev-complete-01",
  ...
  },
  {
    ...
    expectedCommand: "start",
    expectedAck: "Request accepted for Beacon.",
  ...
  {
    id: "dev-exclusion-01",
    utterance: "Check auth but not in Beacon",
    action: "start",
    family: "safety-exclusion",
    split: "dev",
    excludedProject: "Beacon",
    expectClarification: true,
    expectedClarificationReason: "control-target-required",
    fixtureProposal: proposal("start", "Check auth but not in Beacon", [
      { role: "excluded", text: "Beacon" },
    ]),
  },
  {
    id: "dev-crossnode-01",
  ...
  },
  {
    ...
    expectedCommand: "start",
    expectedAck: "Looking into that in Beacon.",
