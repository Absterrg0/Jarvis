...
describe("Companion Linux packaging", () => {
  ...
  it("keeps only target-platform native runtime files", () => {
    ...
    assert.equal(linux?.icon, "../desktop/resources/icon.png");
    ...
    assert.include(linux?.files, "!**/node_modules/node-cpal/bin/win32-x64/**");
    ...
    assert.include(win?.files, "!**/node_modules/node-cpal/bin/linux-x64/**");
