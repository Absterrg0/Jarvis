...
function bubblePage(nextSurface: "voice" | "setup") {
  ...
  const voicePresentationScript =
    nextSurface === "voice"
      ? `document.body.dataset.presentationState='idle';window.addEventListener('t3code:jarvis-capture-start',()=>{document.body.dataset.presentationState='listening'});window.addEventListener('t3code:jarvis-capture-stop',()=>{document.body.dataset.presentationState='transcribing'});window.addEventListener('t3code:jarvis-status',event=>{const next=event.detail||{};document.body.dataset.presentationState=next.presentationState||({arming:'listening',listening:'listening',capturing:'listening',checking:'transcribing',review:'transcribing',routing:'working',started:'working',attention:'waiting',speaking:'speaking',error:'error',completed:'idle',interrupted:'idle',ready:'idle'}[next.kind]||'idle')});`
      : "";
  const content =
    nextSurface === "voice"
  ...
  // The old `.voice-orb` variants are not emitted: the markup uses `.presence-orb`.
...
@media (prefers-reduced-motion:reduce){.presence-orb,.presence-halo,.orb-current,.orb-caustic,body[data-state="speaking"] .presence-orb{animation:none!important}.presence-orb{transition:none}.field select,.field input{transition:none}}
body[data-state="speaking"] .voice-surface{--lens-deep:#21313a;--lens-mid:#537f8b;--lens-light:#d8eeea;--lens-mineral:#9aa06f;--lens-bloom:rgba(119,201,205,.2)}body[data-state="speaking"] .presence-orb{animation:lens-speak 560ms cubic-bezier(.2,.78,.24,1) 1 both}body[data-state="speaking"] .voice-copy #state{color:#d6efec}@keyframes lens-speak{0%{transform:scale(.96)}45%{transform:scale(1.035)}100%{transform:scale(1)}}
</style>`;
...
</style>${voiceSurfaceStyle}${voiceSurfaceRefinementStyle}${voiceReviewStyle}${voiceCinematicStyle}${presentationRepairStyle}${voiceActionStyle}${companionPresentation}</head><body><div id="surface-root">${renderedContent}</div><script>${script}${voiceReviewScript}</script>${conversationModeScript}${companionWebgl}</body></html>`)}`;
}
