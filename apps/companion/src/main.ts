...
function showCompanionStatus(status: {
  ...
  const hideDelay = voiceOverlayAutoHideDelay(status);
  if (hideDelay === undefined) return;
  hideBubbleAbort?.abort();
  const controller = new AbortController();
  hideBubbleAbort = controller;
  void Timers.setTimeout(hideDelay, undefined, { signal: controller.signal })
    .then(() => {
