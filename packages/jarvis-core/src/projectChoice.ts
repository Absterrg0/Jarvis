...
export interface JarvisProjectChoiceCandidate {
...
export type JarvisProjectChoiceMatch = {
...
const foldJarvisChoiceText = (value: string): string =>
  value
    .trim()
...
function candidateNames(
  candidate: JarvisProjectChoiceCandidate,
  ...
  return [
    ...
    .map((value) => foldJarvisChoiceText(value))
 ...
 * instead of guessing.
 */
function fuzzyJarvisProjectChoice(
  answer: string,
  candidates: ReadonlyArray<JarvisProjectChoiceCandidate>,
): JarvisProjectChoiceMatch | null {
  const probes = [answer, ...answer.split(/\s+/u)]
    .map((probe) => ({ text: probe, folded: foldJarvisChoiceText(probe).replace(/\s+/gu, "") }))
    ...
    for (const { compact } of candidateNames(candidate)) {
      for (const probe of probes) {
        const distance = boundedJarvisEditDistance(
          compact,
          probe.folded,
          jarvisChoiceDistanceCap(compact, probe.folded),
        );
 ...
 */
export function resolveJarvisProjectChoice(input: {
  readonly answer: string;
  readonly candidates: ReadonlyArray<JarvisProjectChoiceCandidate>;
  readonly acceptsAffirmation?: boolean;
}): JarvisProjectChoiceMatch | null {
  const raw = input.answer.trim();
  const answer = foldJarvisChoiceText(raw);
  ...
  return fuzzyJarvisProjectChoice(raw, input.candidates);
}
