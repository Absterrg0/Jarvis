...
function buildJarvisSemanticPromptContext(
  ...
  const localPending = describePendingJarvisRequest(input.contextThread?.activities);
  const pendingRequest =
    localPending !== "none"
      ? localPending
      : input.pendingReplyTask === undefined
        ? "none"
        : `${describePendingJarvisRequest(input.pendingReplyThread?.activities)} in task "${input.pendingReplyTask.title}": answer it with continue`;
  return [
...
const JARVIS_SEMANTIC_PROMPT_RULES: ReadonlyArray<string> = [
...
const JARVIS_FAST_SEMANTIC_PROMPT_RULES: ReadonlyArray<string> = [
  "Route one ARIS request to one JSON proposal. Output ONLY this JSON, no prose and no tools:",
  '{"action":"start|continue|steer|queue|stop|status|review|reroute|focus-project|focus-task|list-projects|converse|unsupported","refs":[{"span":{"start":number,"end":number,"text":string},"role":"destination|task|subject|excluded|correction|provider","value":string}],"model":string|null,"effort":string|null,"answer":string|null}',
  "refs cite exact UTF-16 spans of the Original transcript: text is the exact copied slice and value echoes it; the host rejects any inexact span. destination is only the full routing wrapper (for example 'in Alertify'); only destination or correction names the project. task is a work title, provider a requested runner; subject and excluded never route. At most one destination or correction, one task, one provider.",
  "Use exact catalog names; never invent internal IDs. Use null for unspecified model, effort, and answer; answer is required only for converse (at most 400 characters).",
  ...
  '- "Stop auth, then create a deployment task" => unsupported.',
...
export function buildJarvisSemanticPrompt(
  ...
  return [
    ...JARVIS_SEMANTIC_PROMPT_RULES,
    ...buildJarvisSemanticPromptContext(input, prepared),
  ].join("\n");
}
 ...
 */
export function buildJarvisFastSemanticPrompt(
  input: JarvisCommandContext,
  prepared: Extract<PreparedJarvisSemanticTurn, { status: "ready" }>,
): string {
  return [
    ...JARVIS_FAST_SEMANTIC_PROMPT_RULES,
    ...buildJarvisSemanticPromptContext(input, prepared, { tasks: 4, objectiveChars: 100 }),
  ].join("\n");
}
