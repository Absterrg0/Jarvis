import type { JarvisCommandContext } from "./command.ts";
import type { JarvisSemanticProposal, SemanticRef, SemanticRole } from "./semanticEvidence.ts";
import {
  isInJarvisNegationScope,
  normalizeDestinationPhrase,
  stripDestinationQuotes,
  stripJarvisInvocation,
...
/**
 * Bounded closed grammar for one Jarvis turn. Proposes only for a small
 ...
 *
 * Only ARIS (current invocation) and Jarvis (compatibility) open an
 * invocation wrapper. Every other name resolves through bounded catalogs.
 */

export type LocalGrammarProposal = JarvisSemanticProposal;
 ...
 */
export type JarvisLocalTierMode = "disabled" | "restricted";
export interface JarvisLocalTierConfig {
  readonly mode: JarvisLocalTierMode;
}
export const JARVIS_LOCAL_TIER_DEFAULT: JarvisLocalTierConfig = { mode: "disabled" };

export function tryJarvisLocalTier(_input: {
  readonly source: string;
  readonly context: JarvisCommandContext;
  readonly config?: JarvisLocalTierConfig;
...
function buildCatalogsFromContext(context: JarvisCommandContext): {
  projects: CatalogProject[];
  tasks: CatalogTask[];
} {
  const grouped = groupJarvisAliasesByProject(context.aliases);
...
function findLeadingDestination(
  ...
  for (const project of ordered) {
    ...
    for (const name of names) {
      ...
      if (isInJarvisNegationScope(wrapperStart - offset, rest)) {
...
function findTrailingDestination(
  ...
  for (const project of ordered) {
    ...
    for (const name of names) {
      ...
      const text = source.slice(start, end);
      if (text.length === 0 || text.length > MAX_DESTINATION_SPAN) continue;
      if (isInJarvisNegationScope(startInRest, rest)) {
      ...
      const remainderWithoutInvocation = stripJarvisInvocation(remainder).rest.trim();
