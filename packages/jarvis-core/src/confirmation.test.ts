...
describe("spoken confirmation negation", () => {
  it.each(["do not allow it", "don't approve it", "never allow that", "cannot approve it"])(
    "declines a negated approval answer instead of consenting: %s",
    (utterance) => {
      expect(resolveSpokenApprovalDecision(utterance)).toBe("decline");
    },
  );

  it.each(["that's not right", "no, not that one", "never mind"])(
    "declines a negated voice confirmation instead of accepting: %s",
    (utterance) => {
