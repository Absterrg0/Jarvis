 ...
 * scripts/jarvis-extract). The semantic contract takes UTF-16 source spans
 ...
 *
 * Roles-v1 labels (scripts/jarvis-extract, train --label-schema roles-v1):
...
const emit = (proposal: JarvisSemanticProposal): ExtractionAdapterResult => {
  try {
    decodeJarvisSemanticProposal(proposal);
  } catch {
    return reject(
      "malformed",
