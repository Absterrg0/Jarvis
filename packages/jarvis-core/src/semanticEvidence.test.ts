...
const catalogs: SemanticEvidenceCatalogs = {
  projects: [
    { id: ProjectId.make("project-jarvis"), title: "Jarvis", names: ["Jarvis", "jarvis"] },
    {
...
describe("cardinality without conjunction reading", () => {
  ...
  it("rejects two destination refs and destination plus correction", () => {
    const source = "In Rivvl in Jarvis fix it";
    expect(
      validate(source, [
        { ...ref(source, "destination", "In Rivvl", "Rivvl") },
        { ...ref(source, "destination", "in Jarvis", "Jarvis") },
      ]),
    ).toMatchObject({ status: "malformed", kind: "cardinality" });
    const repair = "I meant VPS not Rivvl in Jarvis";
    expect(
      validate(repair, [
        { span: cite(repair, "VPS"), role: "correction", value: "VPS" },
        { span: cite(repair, "in Jarvis"), role: "destination", value: "Jarvis" },
      ]),
    ).toMatchObject({ status: "malformed", kind: "cardinality" });
  });
  ...
  it("decodes one inference with explicit nulls and quoted spans", () => {
    ...
    const decoded = decodeJarvisSemanticProposal({
      ...
      refs: [
        ...
        { span: cite(source, "in Jarvis repo"), role: "destination", value: "Jarvis" },
      ],
      model: null,
      effort: null,
      answer: null,
