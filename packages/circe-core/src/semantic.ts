...
const CIRCE_SEMANTIC_PROMPT_RULES: ReadonlyArray<string> = [
  ...
  '- "Stop authentication, then create a deployment task" => action sequence with steps: stop authentication, then start a deployment task. Each step is a complete single command and the host validates them all before dispatching any.',
...
const CIRCE_FAST_SEMANTIC_PROMPT_RULES: ReadonlyArray<string> = [
  ...
  "A weather or local-time question is lookup with lookup.location copied verbatim from the transcript and day now|today|tomorrow; a request to open a named site or URL is open-website with website set to it. Neither takes project or task refs. A request that combines either with other work is unsupported.",
  'Two genuinely separate commands use action "sequence" with a steps array of up to four complete single commands (same fields as the top level; steps never nest). The host validates every step before dispatching any. Never use unsupported for two commands you can express as steps.',
  "Examples:",
