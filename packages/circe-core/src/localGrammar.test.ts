...
const authTask: CirceCommandTask = {
...
function context(overrides: Partial<CirceCommandContext> = {}): CirceCommandContext {
  return {
...
const ready = (input: CirceCommandContext) => {
  const prepared = prepareCirceSemanticTurn({ ...input, utterance: input.utterance });
...
describe("bounded local grammar positives", () => {
  ...
  it("proposes single-token auth in Rivvl with a canonical destination wrapper", () => {
    ...
    const interpreted = interpretCirceCommand(
  ...
  it("accepts Circe invocation and Circe compatibility with source preserved", () => {
    for (const invocation of ["Circe, check auth in Rivvl", "Circe, check auth in Rivvl"]) {
      ...
      expect(joined).toContain(invocation.startsWith("Circe") ? "Circe," : "Circe,");
    }
  });
  ...
  it("resolves through catalogs, never substrings", () => {
    ...
    const outcome = tryBoundedLocalGrammar({
      source,
      context: context({ utterance: source, projects: [app, circe] }),
    });
...
describe("closed start matrix over names", () => {
  const run = (source: string, overrides: Partial<CirceCommandContext> = {}) =>
...
describe("local-tier seam stays a disabled hook", () => {
  it("defaults to disabled and never proposes", () => {
    ...
    const result = tryCirceLocalTier({
