...
describe("clarification answer command remainder", () => {
  it.each([
    ...
    (answer, matched) => {
      expect(circeClarificationAnswerHasCommandRemainder({ answer, matchedText: matched })).toBe(
        true,
      );
    },
  );
  ...
  it("fails closed when the matcher consumed nothing", () => {
    expect(
      circeClarificationAnswerHasCommandRemainder({ answer: "anything", matchedText: "" }),
    ).toBe(true);
  });
});
