...
import { createCirceTurnTiming } from "./timing.ts";
...
describe("Circe turn timing", () => {
  it("measures elapsed time between two marks", () => {
    const timing = createCirceTurnTiming(controlledClock([100, 160]));
  ...
  it("keeps the first write when a boundary repeats", () => {
    const timing = createCirceTurnTiming(controlledClock([100, 200]));
  ...
  it("reports undefined instead of zero for missing marks", () => {
    const timing = createCirceTurnTiming(controlledClock([100]));
    timing.mark("semantic-start");
