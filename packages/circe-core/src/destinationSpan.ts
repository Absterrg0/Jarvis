...
/**
 * Invocation wrapper owned by source text: only Circe and Circe open one.
 ...
 * each other.
 */
export function stripCirceInvocation(source: string): {
  readonly rest: string;
  readonly offset: number;
} {
  const match = /^\s*(Circe|Circe)\s*(?:[,:\-–—]\s*|\s+)/iu.exec(source);
...
export function isInCirceNegationScope(wrapperStartInRest: number, rest: string): boolean {
 ...
 */
export function isCirceNegatedSpan(source: string, spanStart: number): boolean {
