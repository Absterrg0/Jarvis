...
describe("describeApproval", () => {
  it("describes a structured command request literally", () => {
    expect(
      ...
      spoken: "The agent is requesting permission to run the provided command in Circe. Allow it?",
      risk: "unknown",
      rawDetail: "pnpm test",
    });
  });
  ...
  it("falls back to a provider request when metadata is absent", () => {
    expect(describeApproval({ detail: "rm -rf /", projectTitle: "Circe" })).toEqual({
      spoken: "The provider is requesting approval in Circe. Allow it?",
      risk: "unknown",
      rawDetail: "rm -rf /",
    });
  });
