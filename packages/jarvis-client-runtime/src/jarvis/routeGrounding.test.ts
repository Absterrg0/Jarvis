...
function proposal(
  action: JarvisSemanticProposal["action"],
  refs: JarvisSemanticProposal["refs"],
...
describe("proposal-first execute route grounding", () => {
  ...
  it("never routes subject-only mentions", () => {
    ...
    const withJarvis = source.indexOf("Jarvis");
    expect(
      resolveJarvisProposalExecuteRoute(
        catalog,
        source,
        proposal("start", [
          ...
          {
            span: { start: withJarvis, end: withJarvis + "Jarvis".length, text: "Jarvis" },
            role: "subject",
            value: "Jarvis",
          },
        ]),
  ...
  it("never swaps a qualified pinned followup to a mentioned project", () => {
    ...
    const pinned = {
      projectRef: { nodeId: DESKTOP, projectId: ProjectId.make("jarvis-desktop") },
      contextThreadId: ThreadId.make("thread-jarvis"),
    };
    expect(
      resolveJarvisProposalExecuteRoute(
        catalog,
...
describe("route coverage confirmation under partial catalogs", () => {
  ...
  const partialCatalog: JarvisMeshCatalog = {
    nodes: [
  ...
  const fullCatalog: JarvisMeshCatalog = {
    ...partialCatalog,
    nodes: [
      { nodeId: LOCAL, label: "Local", reachability: "online" },
