 ...
 */
export function formatJarvisVoiceReceipt(transcript: string): string | null {
 ...
 */
export function formatJarvisVoiceDispatching(transcript: string): string {
  return `Heard "${truncateTranscript(transcript)}", checking...`;
}

export type JarvisVoiceEnqueueResult = "enqueued" | "duplicate" | "full" | "empty";

/** Only a newly enqueued capture earns a receipt; retries stay silent. */
export function shouldEmitJarvisVoiceReceipt(result: JarvisVoiceEnqueueResult): boolean {
  return result === "enqueued";
}

export type JarvisVoiceFeedbackKind = "working" | "needs-input" | "error" | "done";

export interface JarvisVoiceWaitingView {
 ...
 * after acceptance.
 */
export function buildJarvisVoiceWaitingView(input: {
  readonly busy: boolean;
  readonly awaitingAnswer: boolean;
  readonly feedbackKind: JarvisVoiceFeedbackKind | null;
  readonly feedbackText: string | null;
  readonly targetLabel: string | null;
  readonly targetAvailable: boolean;
}): JarvisVoiceWaitingView | null {
 ...
 * claiming it stopped.
 */
export function resolveJarvisVoiceCancelMessage(input: {
