...
describe("jarvis voice waiting", () => {
  it("emits no receipt for an empty transcript", () => {
    expect(formatJarvisVoiceReceipt("   ")).toBeNull();
    expect(formatJarvisVoiceReceipt("")).toBeNull();
  });

  it("keeps the receipt short with the transcript intact", () => {
    const receipt = formatJarvisVoiceReceipt("Fix the login bug");
    expect(receipt).toBe('Heard: "Fix the login bug"');
    const long = formatJarvisVoiceReceipt(`Fix ${"very ".repeat(60)}long bug`);
  ...
  it("emits a receipt only for a newly enqueued capture", () => {
    expect(shouldEmitJarvisVoiceReceipt("enqueued")).toBe(true);
    expect(shouldEmitJarvisVoiceReceipt("duplicate")).toBe(false);
    expect(shouldEmitJarvisVoiceReceipt("full")).toBe(false);
    expect(shouldEmitJarvisVoiceReceipt("empty")).toBe(false);
  });
  ...
  it("marks the target provisional while the semantic call is unaccepted", () => {
    const view = buildJarvisVoiceWaitingView({
      busy: true,
      awaitingAnswer: false,
      feedbackKind: "working",
      feedbackText: 'Heard: "Fix the bug"',
  ...
  it("keeps dispatch text provisional and silent", () => {
    const text = formatJarvisVoiceDispatching("Fix the login bug");
  ...
  it("tells the truth when the current request is already submitted", () => {
    expect(resolveJarvisVoiceCancelMessage({ inFlight: true, discardedQueued: 2 })).toBe(
      "Discarded 2 queued requests. The current request is already submitted and keeps running.",
    );
    expect(resolveJarvisVoiceCancelMessage({ inFlight: true, discardedQueued: 0 })).toBe(
      "The current request is already submitted and keeps running.",
    );
    expect(resolveJarvisVoiceCancelMessage({ inFlight: false, discardedQueued: 1 })).toBe(
      "Discarded 1 pending request.",
    );
    expect(resolveJarvisVoiceCancelMessage({ inFlight: false, discardedQueued: 0 })).toBe(
      "Nothing to cancel.",
    );
  });
});
