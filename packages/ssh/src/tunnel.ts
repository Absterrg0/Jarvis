...
if command -v circe >/dev/null 2>&1; then
  exec circe "$@"
fi
...
# the npx cache without a circe executable. \`npx --yes\` then exits 0 without
...
require_installed_t3_cli() {
  if ! T3_CLI_PATH="$("$@" -- sh -c 'command -v circe')"; then
  ...
  printf 'Remote host installed %s but npm produced no circe executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@T3_PACKAGE_SPEC@@ >&2
  return 1
}
...
printf 'Remote host is missing the circe CLI and could not install @@T3_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "@absterrg0/circe@latest");
