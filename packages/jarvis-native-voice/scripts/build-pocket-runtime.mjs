...
void ptt_stream_end(void* stream_ctx) {`,
  ...
  {
    name: "in-place stream cancellation without joining or freeing",
    old: `const char* ptt_stream_error(void* stream_ctx) {
    if (!stream_ctx) return "";
    auto* ctx = static_cast<ptt_stream_ctx*>(stream_ctx);
    std::lock_guard<std::mutex> lock(ctx->mtx);
    return ctx->error.c_str();
...
void ptt_stream_cancel(void* stream_ctx) {
  ...
  {
    name: "voice pre-encoding for fast first synthesis and cancellation",
    old: `    const Config& config() const { return cfg_; }`,
    new: `    const Config& config() const { return cfg_; }
    void prepare_voice(const std::string& path) { get_voice(path); }`,
  },
  {
    name: "voice preparation entry point",
    old: `double ptt_warmup(void* handle) {`,
    new: `int ptt_prepare_voice(void* handle, const char* voice) {
    if (!handle || !voice) return -1;
    try {
        static_cast<pocket_tts::PocketTTS*>(handle)->prepare_voice(voice);
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "[pocket-tts] voice error: " << e.what() << "\\n";
        return -1;
    }
}
...
double ptt_warmup(void* handle) {`,
  ...
  {
    name: "no ORT thread spinning (bursty voice use; fast teardown)",
    old: `        auto make_opts = [](int threads) {
    ...
    new: `        auto make_opts = [](int threads) {
            Ort::SessionOptions opts;
            opts.SetIntraOpNumThreads(threads);
            opts.SetInterOpNumThreads(1);
            opts.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);
  ...
  {
    name: "no ORT thread spinning on arena-less sessions either",
    old: `        auto make_opts_no_arena = [](int threads) {
            Ort::SessionOptions opts;
            opts.SetIntraOpNumThreads(threads);
            opts.SetInterOpNumThreads(1);
            opts.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);
            opts.DisableMemPattern();`,
    new: `        auto make_opts_no_arena = [](int threads) {
            Ort::SessionOptions opts;
            opts.SetIntraOpNumThreads(threads);
            opts.SetInterOpNumThreads(1);
            opts.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);
