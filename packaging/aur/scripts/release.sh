...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Circe-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit circe-icon.png
git config user.name 'circe-ci'
git config user.email 'circe-ci@users.noreply.github.com'
git add -A
