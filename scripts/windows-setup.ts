...
// User-visible installer copy is ARIS. The task name, install dir,
// registry paths, Jarvis.exe targets, and uninstall identity stay Jarvis
...
/** Stop only ARIS processes backed by the preserved Jarvis.exe owned by this install. */
export function renderWindowsOwnedProcessStopPs1(): string {
  return [
    ...
    '  Write-Error "Owned ARIS processes remain after stop: $ids"',
    "  exit 1",
    "} catch {",
    '  Write-Error "Could not safely stop owned ARIS processes: $($_.Exception.Message)"',
    "  exit 1",
    "}",
    "",
  ].join("\r\n");
...
export function renderWindowsSetupNsi(input: {
  ...
  return [
    ...
    '!define MUI_WELCOMEPAGE_TITLE "Welcome to ARIS Setup"',
    ...
    '!define MUI_FINISHPAGE_RUN_TEXT "Launch ARIS"',
    '!include "MUI2.nsh"',
    '!include "LogicLib.nsh"',
    '!include "nsDialogs.nsh"',
    '!include "FileFunc.nsh"',
    `Name "ARIS ${input.version}"`,
    `OutFile ${nsiQuote(input.outputPath)}`,
    `BrandingText "ARIS ${input.version}"`,
    ...
    `VIAddVersionKey /LANG=1033 "ProductName" "ARIS"`,
    `VIAddVersionKey /LANG=1033 "ProductVersion" "${input.version}"`,
    `VIAddVersionKey /LANG=1033 "FileDescription" "ARIS Node setup"`,
    ...
    "    Abort",
    "  ${EndIf}",
    '  ${NSD_CreateLabel} 0 0 100% 18u "Choose how this Windows device runs ARIS"',
    "  Pop $0",
    ...
    "apps_prompt:",
    '  MessageBox MB_ICONEXCLAMATION|MB_OKCANCEL "Close ARIS before continuing. The installer will request it to exit." IDOK apps_close IDCANCEL apps_abort',
    "apps_abort:",
    "  Abort",
    "apps_close:",
    ...
    '  MessageBox MB_ICONSTOP "ARIS could not stop its existing processes safely. Close ARIS and try again."',
    "  Abort",
    "reset_stop_failed:",
    ...
    '  MessageBox MB_ICONSTOP "ARIS could not stop the existing headless runtime. Your installation was left in place."',
    "  Abort",
    "reset_stop_done:",
    "SectionEnd",
    "",
    ...
    `  WriteRegStr HKCU "${WINDOWS_SETUP_UNINSTALL_REGISTRY_KEY}" "DisplayName" "ARIS"`,
    ...
    "state_gui:",
    '  Delete "$PROFILE\\.jarvis\\runtime\\windows-stop.marker"',
    '  CreateDirectory "$SMPROGRAMS\\ARIS"',
    `  CreateShortCut "$DESKTOP\\ARIS.lnk" "$INSTDIR\\desktop\\Jarvis.exe"`,
    `  CreateShortCut "$SMPROGRAMS\\ARIS\\ARIS.lnk" "$INSTDIR\\desktop\\Jarvis.exe"`,
    ...
    '  MessageBox MB_ICONSTOP "ARIS could not validate the staged payload. Your existing installation was left in place."',
    "  Abort",
    "FunctionEnd",
    "",
    ...
    "  Quit",
    "restore_interactive:",
    '  MessageBox MB_ICONSTOP "ARIS could not commit the staged payload. Your existing installation was restored."',
    "  Abort",
    "restore_failed:",
    '  RMDir /r "$INSTDIR\\.incoming"',
    ...
    '  MessageBox MB_ICONSTOP "ARIS could not fully roll back the staged payload. Recovery files, if present, remain at $INSTDIR\\.previous."',
    "  Abort",
    "FunctionEnd",
    "",
    'Section "Uninstall"',
    ...
    '  MessageBox MB_ICONSTOP "ARIS could not stop the headless runtime. The installation was left in place."',
    "  Abort",
    ...
    '  MessageBox MB_ICONSTOP "ARIS could not stop its processes safely. The installation was left in place."',
    "  Abort",
    "un_stop_headless_done:",
    '  Delete "$DESKTOP\\ARIS.lnk"',
    '  Delete "$SMPROGRAMS\\ARIS\\ARIS.lnk"',
    '  RMDir "$SMPROGRAMS\\ARIS"',
    '  Delete "$DESKTOP\\Jarvis.lnk"',
    '  Delete "$SMPROGRAMS\\Jarvis\\Jarvis.lnk"',
    '  RMDir "$SMPROGRAMS\\Jarvis"',
