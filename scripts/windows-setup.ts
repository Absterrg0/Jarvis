...
export function renderWindowsNodeSupervisorMjs(): string {
  return [
    ...
    'const home = process.env.T3CODE_HOME || join(process.env.USERPROFILE || "", ".circe");',
...
export function renderWindowsNodeLauncherCmd(): string {
  return [
    ...
    `set "T3CODE_HOME=%USERPROFILE%\\.circe"`,
    `set "CIRCE_NODE_PRESET=headless"`,
    `set "CIRCE_NODE_STOP=%T3CODE_HOME%\\runtime\\windows-stop.marker"`,
    'cd /d "%~dp0"',
    ":run_supervisor",
