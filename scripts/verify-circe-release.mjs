...
export const expectedCirceReleaseAssets = (version) => [
  ...
  `Circe-${version}-x86_64.AppImage.provenance.json`,
  `Circe-${version}-x86_64.AppImage.sha256`,
  `Circe-Headless-Node-${version}-linux-arm64.tar.gz`,
  `Circe-Headless-Node-${version}-linux-arm64.tar.gz.provenance.json`,
  `Circe-Headless-Node-${version}-linux-arm64.tar.gz.sha256`,
  `Circe-Headless-Node-${version}-linux-x64.tar.gz`,
  `Circe-Headless-Node-${version}-linux-x64.tar.gz.provenance.json`,
  `Circe-Headless-Node-${version}-linux-x64.tar.gz.sha256`,
  `Circe-Setup-${version}-win-x64.exe`,
  `Circe-Setup-${version}-win-x64.exe.sha256`,
  `Circe-Setup-${version}-win-x64.exe.manifest.json`,
  `Circe-Setup-${version}-win-x64.exe.manifest.json.sha256`,
...
export function verifyCirceReleaseDirectory(directory, { version, sourceCommit }) {
  const expected = expectedCirceReleaseAssets(version).sort();
  ...
  const linuxArtifact = `Circe-${version}-x86_64.AppImage`;
  const macArtifacts = ["arm64", "x64"].map((arch) => `Circe-${version}-${arch}.dmg`);
  const setupArtifact = `Circe-Setup-${version}-win-x64.exe`;
  ...
  assertEqual(sha256(file("Circe-Setup.exe")), sha256(file(setupArtifact)), "restored setup alias");
...
export function writeCirceSha256Sums(directory) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!directory || !version || !sourceCommit) {
    throw new Error(
      "usage: node scripts/verify-circe-release.mjs <directory> <version> <source-commit> [--write-sha256sums]",
    );
  }
  verifyCirceReleaseDirectory(directory, { version, sourceCommit });
  if (option === "--write-sha256sums") writeCirceSha256Sums(directory);
}
