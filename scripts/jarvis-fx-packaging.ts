 ...
 */
export const JARVIS_FX_VERSION = "0.0.8";
export const JARVIS_FX_PIN = `v${JARVIS_FX_VERSION}`;
export const JARVIS_FX_RESOURCE_DIR = "fx";
export const DESKTOP_FX_EXTRA_RESOURCE = {
  from: `apps/desktop/prod-resources/${JARVIS_FX_RESOURCE_DIR}`,
  to: JARVIS_FX_RESOURCE_DIR,
} as const;

export type JarvisFxBuildPlatform = "linux" | "mac" | "win";
export type JarvisFxBuildArch = "x64" | "arm64";

/** Release asset for the target, or null when fx does not publish one. */
export function jarvisFxReleaseAsset(
  platform: JarvisFxBuildPlatform,
  arch: JarvisFxBuildArch,
): string | null {
...
const releaseBase = (): string =>
  `https://github.com/vercel-labs/fx/releases/download/${JARVIS_FX_PIN}`;
...
async function fetchVerifiedArchive(asset: string): Promise<Buffer> {
  const cacheDir = NodePath.join(NodeOS.homedir(), ".cache", "jarvis-fx", JARVIS_FX_PIN);
 ...
 */
export async function stageJarvisFxResources(input: {
  readonly platform: JarvisFxBuildPlatform;
  readonly arch: JarvisFxBuildArch;
  readonly stageProdResourcesDir: string;
}): Promise<string | null> {
  const asset = jarvisFxReleaseAsset(input.platform, input.arch);
  if (asset === null) return null;
  const destinationDir = NodePath.join(input.stageProdResourcesDir, JARVIS_FX_RESOURCE_DIR);
