...
import { expectedCirceReleaseAssets, sha256, verifyCirceReleaseDirectory, writeCirceSha256Sums } from "./verify-circe-release.mjs";
...
function makeFixture() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "circe-release-verifier-"));
  const assets: string[] = expectedCirceReleaseAssets(version);
  for (const name of assets) {
    if (
      !name.endsWith(".sha256") &&
      !name.endsWith(".provenance.json") &&
      name !== "Circe-Setup.exe"
    ) {
      NodeFS.writeFileSync(NodePath.join(directory, name), `fixture:${name}`);
    }
  }
  const setup = `Circe-Setup-${version}-win-x64.exe`;
  NodeFS.copyFileSync(NodePath.join(directory, setup), NodePath.join(directory, "Circe-Setup.exe"));
  const linux = `Circe-${version}-x86_64.AppImage`;
  ...
  for (const arch of ["arm64", "x64"]) {
    const artifact = `Circe-${version}-${arch}.dmg`;
    NodeFS.writeFileSync(
  ...
  for (const arch of ["x64", "arm64"]) {
    const artifact = `Circe-Headless-Node-${version}-linux-${arch}.tar.gz`;
    NodeFS.writeFileSync(
      NodePath.join(directory, `${artifact}.provenance.json`),
      JSON.stringify({
        format: 1,
  ...
  const manifest = `Circe-Setup-${version}-win-x64.exe.manifest.json`;
  const provenance = `Circe-Setup-${version}-win-x64.exe.provenance.json`;
  NodeFS.writeFileSync(
...
describe("Circe release staging verifier", () => {
  it("accepts the exact cross-platform asset set and writes a deterministic manifest", () => {
    const directory = makeFixture();
    try {
      verifyCirceReleaseDirectory(directory, { version, sourceCommit });
      const names = writeCirceSha256Sums(directory);
  ...
  it("rejects an unexpected or missing release asset before publication", () => {
    const directory = makeFixture();
    try {
      NodeFS.unlinkSync(NodePath.join(directory, "Circe-Setup.exe"));
      NodeFS.writeFileSync(
        NodePath.join(directory, `Circe-${version}-arm64.zip`),
  ...
  it("rejects a missing or misnamed macOS DMG before publication", () => {
    const directory = makeFixture();
    const artifact = `Circe-${version}-x64.dmg`;
    try {
      NodeFS.unlinkSync(NodePath.join(directory, artifact));
      NodeFS.writeFileSync(
        NodePath.join(directory, `Circe-${version}-x64.zip`),
  ...
  it("rejects non-canonical checksum sidecars", () => {
    const directory = makeFixture();
    const artifact = `Circe-${version}-x86_64.AppImage`;
    ...
    try {
      ...
      try {
        const manifestPath = NodePath.join(
          validDirectory,
          `Circe-Setup-${version}-win-x64.exe.manifest.json`,
        );
        ...
        const provenancePath = NodePath.join(
          validDirectory,
          `Circe-Setup-${version}-win-x64.exe.provenance.json`,
        );
        ...
        assert.throws(
          () => verifyCirceReleaseDirectory(validDirectory, { version, sourceCommit }),
  ...
  it("rejects a mislabeled macOS artifact provenance record", () => {
    const directory = makeFixture();
    const artifact = `Circe-${version}-arm64.dmg`;
  ...
  it("streams chunked digests that match whole-file hashes on large artifacts", () => {
    const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "circe-release-digest-"));
    try {
