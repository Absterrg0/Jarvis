# Jarvis relay self-hosting

> For maintainers. This is how to run your own relay. There is no shared production relay.

The relay is Jarvis-owned. The stack is `JarvisRelay`, the package is `@t3tools/jarvis-relay`, and the production database is `jarvisrelay`. Clients ship with no relay configured. You provide the Cloudflare zones, the database, the Clerk application, and the domains, then point your builds at the result.

Follow [T3 Connect setup](./connect-setup.md) for client-side Clerk details and [relay observability](./relay-observability.md) once the deployment is up.
...
- Create a JWT template. The template name goes to clients as `T3CODE_CLERK_JWT_TEMPLATE`. Its `aud` claim must equal the relay `CLERK_JWT_AUDIENCE` value. The names can stay `t3-relay` and `t3-code-relay` inside your own instance, or pick your own pair, as long as the two sides match.
...
```sh
vp run --filter @t3tools/jarvis-relay deploy --stage prod --yes
```
