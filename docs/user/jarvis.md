...
The API key stays on the node and is never sent to a controller, browser, or phone; those clients only ask the node to start a session. Sessions bill by the second on the node's OpenAI account, and the node opens one only while live conversation is on. A node without a key reports that plainly.
