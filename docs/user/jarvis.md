...
After pairing, Companion opens a compact **Voice defaults** panel. Choose the project for new tasks, then choose a ready provider, model, and any required reasoning level. Companion sends those exact targets with each spoken task unless you explicitly name another provider in the request, so you can usually say the task itself rather than repeating routing details. The project choice is independent of whichever project happens to be visible in T3. Jarvis Host validates the saved choices before it starts work; if one is no longer available, Companion sends you back to defaults instead of guessing.
...
Companion has no normal workspace window. It keeps a hidden authenticated report relay and a tray icon only. Hold `Ctrl+Shift+J` to prepare a local microphone, wait for its soft ready tone, speak, then release to send. The textured command surface shows its listening state, exact final transcript, and selected project before routing directly to Jarvis Host. The task-start path does not go through the hidden workspace page. If a device policy prevents Companion's native hold shortcut, its tray menu clearly says that it has fallen back to tap-to-talk.
