...
The Circe mobile app opens directly into Circe. It can browse paired nodes and projects, open recent work, and send Circe text commands. It does not host projects or run coding providers.

Pair the phone with each Circe environment through the existing T3 connection flow. Tailnet connections use the same authenticated RPC channel as the rest of the mobile app.
...
Known semantic boundaries: a correction that denies a project without settling on one asks which project should receive the task. Joining two independent commands in one turn is unsupported and nothing dispatches. An open-ended or ambiguous request makes no claim: Circe asks instead of guessing.

The app remembers the last valid project. On a fresh install it follows the focused or most recent
Circe task, or selects the project automatically when only one is available. An explicitly named target that is missing stays
unavailable instead of falling back; multiple ambiguous choices still require confirmation
so Circe cannot send work to the wrong machine.
...
Leaving Circe never cancels an already submitted coding task: it continues on its execution node. Return to Circe or open the normal thread screen to inspect durable task state.
