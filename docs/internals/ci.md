...
- **Check**: `vp check` (format and lint), then `vpr typecheck` for the workspace type check,
  plus the desktop pipeline build and preload bundle verification. Circe packaging has its own
...
- [`circe-setup-windows.yml`](../../.github/workflows/circe-setup-windows.yml) runs setup,
  server/controller, and Circe UI tests; builds the role-selecting setup; verifies payload
  ...
  [`circe-release.yml`](../../.github/workflows/circe-release.yml) is the release coordinator. Its
  ...
  `scripts/circe-release-transaction.ts`.
