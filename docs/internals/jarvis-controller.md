...
- Task clarification consumes its exact frame and changes focus in one transaction. Later focus bookkeeping preserves any newer question. Provider, model, and effort clarification carries the selected values and the next typed step across web, desktop, and mobile.
- The mesh catalog belongs to the client runtime. Its subscribers receive each completed node read, targeted refresh preserves peer entries, and unread nodes keep name resolution partial. Mobile checks retained task references against durable thread state on reconnect instead of treating the bounded recent-task list as a complete task catalog.
