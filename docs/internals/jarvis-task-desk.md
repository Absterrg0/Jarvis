...
Project pronunciation is Host-wide rather than part of a device task desk. `JarvisProjectLexicon` appends typed learn/forget events and projects a bounded alias set keyed by real project ID. A correction is learned only after a durable project clarification is consumed, or through the authenticated alias-management operation. The HTTP and WebSocket vocabulary reads join those aliases to the live project shell, so aliases for deleted projects never enter a client vocabulary. Full Desktop and Companion pass bounded live project, repository, provider, and model names into Parakeet's per-stream hotword decoder. Companion refreshes that vocabulary in parallel with capture and resolves it at release, so a current project name improves the same utterance without delaying microphone readiness. After Parakeet decodes the complete push-to-talk utterance, the shared voice lexicon scans bounded project-bearing phrases, including multi-word splits of uncommon names, against real projects only. Exact aliases are repaired and routed immediately. A unique phonetic match enters an explicit confirmation exchange before dispatch; confirmation stores the pronunciation on the Host. Provider/model replacements still require provider-routing language. Companion persists its pending confirmation locally for restart/reconnection, surfaces persistence failures, and exposes learned aliases with idempotent removal actions in its tray.

Voice requests carry `inputMode: voice` through request metadata so routing finishes before provider dispatch. A pure Director preparation step owns the control action, authoritative project grounding, canonical objective, request classification, and execution policy. The client may resolve a strong project-name match early for immediate UI feedback, but the Host repeats the decision against its authoritative project catalog before task creation. An uncertain match returns a clarification instead of opening a thread. The original ASR text is retained as `requestMetadata.sourceUtterance` for diagnostics and retry identity; the visible user message, durable task objective, and provider prompt contain only the canonical request. Routing policy is never injected into the chat message. Spoken inspection requests start in approval-required mode, so a mistaken interpretation cannot silently change files.
...
Kokoro streams sentence-sized WAV chunks through one speech arbiter. It permits one active playback, keeps task acknowledgements in submission order, collapses only stale pending reports, and clears every pending item on explicit barge-in. Production speech uses a slightly slower conversational rate and broader pause scale than the original profile. Every chunk receives 140 ms of real trailing silence before it is written, so player teardown cannot clip the final phoneme. Playback completion still follows the native process exit rather than a guessed wall-clock duration.
