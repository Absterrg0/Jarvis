...
Task identity and navigation policy belong on T3/Jarvis Host so web and desktop clients share the behavior. The multi-node client routes through `EnvironmentRegistry` and sends typed commands to the selected T3 host. The server persists direct current state per authenticated client session. Mobile uses the same shared command-context helper with its paired-environment registry; there is no central discovery service.
