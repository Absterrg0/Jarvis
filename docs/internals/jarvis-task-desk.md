...
Project pronunciation is Host-wide rather than part of a device task desk. `JarvisProjectLexicon` appends typed learn/forget events and projects a bounded alias set keyed by real project ID. A correction is learned only after a durable project clarification is consumed, or through the authenticated alias-management operation. The HTTP and WebSocket vocabulary reads join those aliases to the live project shell, so aliases for deleted projects never enter a client vocabulary. Full and Controller pass bounded live project, repository, provider, and model names into Parakeet's per-stream hotword decoder. After Parakeet finalizes a push-to-talk capture, the client queues the raw recognition envelope; it does not bind that text to an empty or stale catalog. At dequeue, the shared `groundVoiceTurn` Director scans a project-bearing slot against the fresh node-qualified catalog. Exact aliases and clear spelling corrections produce a canonical utterance. A unique phonetic match pauses that same FIFO item for explicit confirmation, so a later capture cannot overtake it. Tied names become node-labeled choices. Confirmation resumes the original request and stores the pronunciation on the Host. Provider/model replacements still require provider-routing language.
