...
3. **In progress:** deterministic voice navigation resolves back, forward, new-conversation, and conservative task matches against bounded real titles, objectives, lifecycle state, and confirmed aliases. Project targeting resolves real titles, workspace basenames, repository names, and conservative phonetic matches. Relationships and learned aliases remain.
4. **In progress:** ambiguous and unknown task matches persist a bounded five-minute frame with only real candidate IDs; ordinal replies and cancellation resolve it before normal intent handling. Phonetic project corrections persist the real candidate and original focus/reroute request so yes/no resumes safely. Learned aliases remain.
