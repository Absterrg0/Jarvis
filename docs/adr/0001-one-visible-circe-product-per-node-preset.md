# One visible Circe product per node preset

Circe node presets are capability choices, not separate products that a user must coordinate. The unified installer must expose one Circe application identity, one launcher, and one uninstall entry: a Full node owns the desktop workspace, local execution, tray, hotkey, and speech; a Controller owns only the controller UI, tray, remote connections, and speech; a Headless node owns only the background runtime and has no voice capability. Full and Controller deliberately install the same signed Circe Desktop distribution. The selected preset changes the capabilities exposed by that distribution; it does not select a second Electron application or a separately updated payload. A Full or Controller installation may use helper processes internally, but Circe must own their lifecycle and connection state, and they must not appear as a second application that needs separate setup or pairing.

Live conversation is the voice path: the renderer owns microphone and speaker media over WebRTC while the node mints the session. `Ctrl+Shift+J` (`Command+Shift+J` on macOS) toggles that session.
...
- Merge every implementation into one Electron process. Rejected as a release requirement because independent task execution is a valuable failure boundary; one product identity does not require one process.
- Build a second Controller executable or embed separate Full and Controller desktop archives. Rejected because it duplicates Electron, signing, update authority, and installer work while recreating two application lifecycles. Controller instead uses the shared Desktop distribution with execution, project, and provider capabilities denied at the server, provider-adapter, and client-routing boundaries.
...
dedicated Circe control-center route is the visible management surface. Explicit open actions must
...
CI, synthetic tests, and package smoke tests can prove protocol wiring and
artifact topology; they cannot prove a microphone, permissions, or device routing. Windows and Linux x64 release candidates therefore require a short real-device
acceptance pass, including live conversation and ordered shutdown/quit verification.
