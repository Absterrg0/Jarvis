...
For the current Full voice stabilization, Windows and Linux x64 use one Electron runtime plus an isolated Node-mode worker, local Parakeet recognition, and the exact shared `node-cpal` `0.1.1` capture path. The product-owned Rust microphone path is no longer a production voice path. Hold-to-talk prefers the OS hook where one exists: Windows uses `uiohook`, Linux prefers the `org.freedesktop.portal.GlobalShortcuts` portal and keeps `uiohook` only as the older X11 fallback (never under Wayland); Electron `globalShortcut` is an explicit tap-toggle fallback when no native hold hook is available. macOS microphone capture is deferred and makes no release claim.
